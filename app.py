...
# --- DESIGN INSPIRADO NA FOTO: CINZA CLEAN COM SALMÃO/TERRACOTA ---
st.markdown("""
    <style>
        @import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@300;400;500;600;700&display=swap');
        
        /* Fundo Cinza Claro e Moderno */
        ...
        .block-container { padding: 25px 40px !important; max-width: 100% !important; }
        
        /* Nome do Salão Centralizado Estilo Clinica Flow */
        ...
        .salon-title {
            font-size: 38px;
            font-weight: 700;
            color: #E08266; /* Tom Salmão/Terracota Elegante */
            letter-spacing: 1px;
            margin: 0;
        }
        .salon-subtitle {
        ...
        /* Dashboard de Informações Redesenhado */
        .info-dashboard {
        ...
        /* Botões de Ação Cinza Arredondados */
        div.nav-bar-buttons button {
            background: #475569 !important; /* Cinza Escuro Clean */
        ...
        div.nav-bar-buttons button:hover {
            background: #E08266 !important; /* Muda para o salmão da foto ao passar o mouse */
            transform: translateY(-1px);
        }
        
        /* Cards de Conteúdo e Calendário Lateral */
        .premium-card {
            background: #FFFFFF;
            border-radius: 20px;
            padding: 25px;
        ...
        /* Slots de Compromisso Estilo Salmão da Foto */
        .timeline-slot {
            background: linear-gradient(135deg, #F5A285 0%, #E08266 100%);
            border-radius: 16px;
            padding: 18px;
            ...
            align-items: center;
            box-shadow: 0 4px 10px rgba(224, 130, 102, 0.15);
        }
...
c_inf1.metric("💵 Dinheiro em Caixa", f"R$ {dinheiro_em_caixa:.2f}")
...
if st.session_state["pagina_atual"] == "Agenda":
    ...
    with st.form("Form Caixa Real"):
        ...
        if st.form_submit_button("Finalizar e Registrar"):
            ...
            recibo_html = f"""<div style="background:#FFF; padding:20px; border-radius:12px; border:1px dashed #E08266; font-family:monospace; color:#333;">
    ...
    if not df_servicos.empty: st.dataframe(df_servicos, use_container_width=True)
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    if not df_produtos.empty: st.dataframe(df_produtos, use_container_width=True)
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    with st.form("FormGastos"):
        ...
        cat_g = st.selectbox("Categoria", ["Infraestrutura", "Aluguel", "Produtos/Insumos", "Marketing", "Outros"])
        if st.form_submit_button("Registrar Saída"):
            ...
            st.rerun()
    df_gastos = pd.read_sql_query("SELECT descricao as 'Descrição', valor as 'Valor (R$)', categoria as 'Categoria', data as 'Data' FROM gastos ORDER BY id DESC", conn)
    ...
    if df_t.empty: st.info("Nenhum cupom emitido.")
    else:
